...
const db = mysql.createConnection({
    host: '8nexot.h.filess.io',
    port: 3307,
    user: 'C237Database_atomfence',
    password: 'a79f5dfcdcbf8993572146c5cf0b2831ad98803d',
    database: 'C237Database_atomfence',
});
...
//store data about user's interaction//
app.use(session({
    secret: 'secret',
    resave: false,
...
const validateRegistration = (req , res, next) => {
    ...
    next(); 
    //if all validation pass, the "next" function is called. allowing the request to proceed to the "next" middleware function or route handler//
}
...
app.get('/login', (req, res) => {
    ...
    res.render('login', {
        ...
        errors: req.flash ('error')//Retrieve error messages from the session and pass them to view
    });
});
...
app.post('/begin', (req, res) => {
    ...
    const sql = 'SELECT * FROM users WHERE email = ? AND  password = SHA(1)';
